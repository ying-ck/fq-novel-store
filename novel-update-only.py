    ...
    while True:
        try:
            if mod==4:
                ...
                it2=it
            res = req.get('https://fanqienovel.com/reader/' + str(it2), headers=headers2)
...
def down_book(it):
    ...
    for i in temp.zj:
        ...
        if f:
            ...
            if p!=0:
                print(p)
                temp.thct+=1
                threading._start_new_thread(down_text,(p,4))
                p=0
            print(i)
            temp.thct+=1
            threading._start_new_thread(down_text,(i,4))
            ft=True
            ...
            if cs>=5:
                ...
                with open(book_json_path, 'w', encoding='UTF-8') as json_file:
                    json.dump(temp.zj, json_file,ensure_ascii=False)
    while temp.thct>0:
        pass
    with open(book_json_path, 'w', encoding='UTF-8') as json_file:
        json.dump(temp.zj, json_file,ensure_ascii=False)
    ...
    if config['save_mode']==1:
        ...
        with open(text_file_path, 'w', encoding='UTF-8') as text_file:
            for chapter_title in temp.zj:
                text_file.write('\n'+chapter_title + fg)
                if config['kg'] == 0:
                    text_file.write(temp.zj[chapter_title] + '\n')
                else:
                    text_file.write(temp.zj[chapter_title].replace('\n', fg) + '\n')
